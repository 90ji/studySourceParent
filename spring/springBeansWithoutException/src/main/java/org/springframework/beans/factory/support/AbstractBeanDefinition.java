...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor implements BeanDefinition, Cloneable {
    ...
    private String scope = SCOPE_DEFAULT;//bean的作用范围

    private boolean abstractFlag = false;//是否是抽象,对应bean属性abstract

    private boolean lazyInit = false;//是否延迟加载,对应bean属性lazy-init

    private int autowireMode = AUTOWIRE_NO;//自动注入模式,对应bean属性lazy-init
    ...
    private boolean autowireCandidate = true;//容器查找自动装配对象时,将考虑该bean

    private boolean primary = false;//自动装配时当出现多个bean候选者时,将作为首选者,对应bean属性primary
    ...
    private boolean lenientConstructorResolution = true;//抛出异常,因为spring无法准确定位哪个构造函数

    private String factoryBeanName;//对应bean属性factory-bean

    private String factoryMethodName;//对应bean属性factory-method

    private ConstructorArgumentValues constructorArgumentValues;//记录构造函数注入属性,对应bean属性constractor-arg

    private MutablePropertyValues propertyValues;//普通属性集合

    private MethodOverrides methodOverrides = new MethodOverrides();//方法重写的持有者记录lookup-method,replaced-method元素

    private String initMethodName;//初始化方法,对应bean属性init-method

    private String destroyMethodName;//销毁方法,对应bean属性destory-method

    private boolean enforceInitMethod = true;//是否执行init-method,程序设置

    private boolean enforceDestroyMethod = true;//是否执行destory-method,程序设置
